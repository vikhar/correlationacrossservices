...
namespace TracingUtil
{
    ...
    public sealed class ServiceEventSource : EventSource
    {
        ...
        private const int UnexpectedErrorOccurredEventId = 50;
        ...
        public void Message(string serviceDetails)
        {
            if (this.IsEnabled())
            {
                WriteEvent(MessageEventId, serviceDetails);
            }
        }
        ...
        public void ServiceRequestStart(string requestTypeName, string message, string serviceDetails)
        {
            WriteEvent(ServiceRequestStartEventId, requestTypeName, message, serviceDetails);
        }
        ...
        public void ServiceRequestStop(string requestTypeName, string message, string serviceDetails, string exception = "")
        {
            WriteEvent(ServiceRequestStopEventId, requestTypeName, message, serviceDetails, exception);
        }

        [Event(ServiceRequestFailedEventId, Level = EventLevel.Error, Message = "Service request '{0}' failed", Keywords = Keywords.Requests)]
        public void ServiceRequestFailed(string requestTypeName, string serviceDetails, string exception)
        {
            WriteEvent(ServiceRequestFailedEventId, serviceDetails, exception);
        }

        [Event(UnexpectedErrorOccurredEventId, Level = EventLevel.Error)]
        public void UnexpectedErrorOccurredEvent(string serviceDetails, string errorMessage, string stackTrace)
        {
            if (IsEnabled())
            {
                WriteEvent(UnexpectedErrorOccurredEventId, serviceDetails, errorMessage, stackTrace);
            }
        }
        #endregion
